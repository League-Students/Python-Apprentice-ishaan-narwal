...
boss_tier, balls_destroyed, balls_needed, score = 1, 0, 30, 0

class Player:
    def __init__(self):
        self.x, self.y = WIDTH // 2, HEIGHT - 50
        self.health, self.max_health = 100, 100
        self.speed, self.cooldown, self.rapid_stacks = 6, 0, 0
...
class Bullet:
    def __init__(self, x, y):
        self.x, self.y, self.speed, self.alive = x, y, 9, True
    def update(self):
        self.y -= self.speed
...
class RedBall:
    def __init__(self):
        self.x, self.y = random.randint(20, WIDTH - 20), random.randint(-100, -20)
        self.speed, self.alive = random.uniform(2, 4), True
    def update(self):
        self.y += self.speed
        if self.y > HEIGHT:
            self.x, self.y = random.randint(20, WIDTH - 20), random.randint(-100, -20)
...
class Boss:
    def __init__(self, tier):
        ...
        self.x, self.y, self.health = WIDTH // 2, 80, tier * 30
        self.max_health, self.direction = self.health, 1
        self.speed = 2 + (tier * 0.5)
        self.shoot_timer, self.shoot_interval = 0, max(15, 50 - (tier * 6))

    def update(self):
        self.x += self.speed * self.direction
        if self.x >= WIDTH - self.size or self.x <= self.size: self.direction *= -1
        self.shoot_timer += 1
...
class Drop:
    def __init__(self, x, y, drop_type):
        self.x, self.y, self.type, self.speed, self.alive = x, y, drop_type, 3, True
    def update(self):
        self.y += self.speed
        if self.y > HEIGHT: self.alive = False

player = Player()
bullets, red_balls, boss_bullets, drops = [], [RedBall() for _ in range(8)], [], []
boss = None
...
def update():
    ...
    if game_state in ["GAME_OVER", "VICTORY"]: return

    for s in stars:
        s["y"] += s["speed"]
        if s["y"] > HEIGHT: s["y"], s["x"] = 0, random.randint(0, WIDTH)
    ...
    if keyboard.space or keyboard.j: player.shoot()
    ...
    if game_state == "BALLS":
        ...
        if balls_destroyed >= balls_needed:
            game_state, boss = "BOSS", Boss(boss_tier)
            boss_bullets.clear()
        ...
        for b in bullets:
            if abs(b.x - boss.x) < (boss.size//2) and abs(b.y - boss.y) < (boss.size//2):
                ...
                if boss.health <= 0:
                    boss = None
                    if boss_tier >= 5: game_state = "VICTORY"
                    else:
                        boss_tier += 1
                        game_state, balls_destroyed = "BALLS", 0
                    break

    for d in drops:
        if abs(d.x - player.x) < 25 and abs(d.y - player.y) < 25:
            d.alive = False
            if d.type == "SHIELD": player.health = min(player.max_health, player.health + 25)
            else: player.rapid_stacks += 1
...
def draw():
    ...
    for s in stars: screen.draw.filled_circle((int(s["x"]), int(s["y"])), 1, (180, 180, 180))
    ...
    for b in bullets: screen.draw.filled_rect(rect((int(b.x - 3), int(b.y - 8)), (6, 16)), (0, 255, 150))
    for d in drops: screen.draw.filled_circle((int(d.x), int(d.y)), 10, (255, 180, 0) if d.type == "RAPID" else (0, 150, 255))
    for bb in boss_bullets: screen.draw.filled_circle((int(bb.x), int(bb.y)), 6, (255, 255, 100))

    if game_state == "BALLS":
        for rb in red_balls: screen.draw.filled_circle((int(rb.x), int(rb.y)), 12, (255, 40, 80))
