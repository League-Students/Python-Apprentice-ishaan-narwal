...
while True:
    ...
    if p_health > 0 and not game_won:
        ...
        # 2. Update Standard Interceptor Enemies
        if boss is None:
            for en in enemies:
                en[1] += en[2]       
                en[3] += 0.08        
                en[0] += int(2.5 * (1 if en[3] % 2 > 1 else -1)) 
        ...
        # 4. Global Hostile Line-Laser Fire Updates & Collisions
        next_b_bullets = []
    ...
    for pt in particles:
        p_hue = COLOR_CYAN if pt[5] == 1 else (COLOR_ORANGE if pt[5] == 2 else (COLOR_CRIMSON if pt[5] == 3 else COLOR_MINT))
        pygame.draw.circle(screen, p_hue, (int(pt[0]), int(pt[1])), 2 if pt[5] == 2 else 3)
    ...
    if boss and not game_won:
        ...
        pygame.draw.rect(screen, (30, 20, 40), (boss[0], boss[1] - 16, bw, 5))
        if boss[3] > 0:
            pygame.draw.rect(screen, COLOR_CRIMSON, (boss[0], boss[1] - 16, int(bw * (boss[3] / boss[4])), 5))

    if p_health > 0 and not game_won:
        current_hue = COLOR_SHIELD if shield_timer > 0 else (COLOR_CYAN if rapid_timer > 0 else COLOR_MINT)
endpygame()